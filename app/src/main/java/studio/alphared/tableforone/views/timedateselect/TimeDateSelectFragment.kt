...
import studio.alphared.tableforone.databinding.FragmentTimeDateSelectBinding
...
import studio.alphared.tableforone.receiver.MealReminderReceiver
...
class TimeDateSelectFragment : Fragment(), DatePickerDialog.OnDateSetListener,
    ...
    private val viewModel : TimeDateSelectViewModel by viewModels{
        TimeDateSelectViewModelFactory(activity?.application)
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        viewModel.showDatePicker.observe(viewLifecycleOwner) { showDatePicker ->
            if (showDatePicker) {
        ...
        viewModel.showTimePicker.observe(viewLifecycleOwner) { showTimePicker ->
            if (showTimePicker) {
                val timePickerDialog = TimePickerDialog(
                    requireContext(), this, viewModel.hour, viewModel.minute,
            ...
            override fun onTransitionStarted(p0: MotionLayout?, p1: Int, p2: Int) {
                runBlocking{
                    launch(Dispatchers.IO) {
                        val mealReminderSave = activityViewModel.getMealReminderToBeSaved(
                            viewModel.myYear, viewModel.myMonth, viewModel.myDay, viewModel.myHour, viewModel.myMinute)
                        val mealIdSaved = viewModel.saveMealReminder(mealReminderSave)
                        //Notification
                        if (mealReminderSave != null) {
                            if (mealIdSaved != null) {
                                createDataNotification(mealIdSaved, mealReminderSave)
                            }
                        }
                    }
                }
    ...
    private fun createDataNotification(mealIdSaved: Long, mealReminderToBeSaved: MealReminder) {
        ...
        datetimeToAlarm.set(MONTH, mealReminderToBeSaved.mealMonth - 1)
        ...
        val intent = Intent(requireActivity().applicationContext, MealReminderReceiver::class.java).apply {
            ...
            val dateTimeConcat = StringBuilder().append(mealReminderToBeSaved.mealYear)
                .append(mealReminderToBeSaved.mealMonth)
                .append(mealReminderToBeSaved.mealDay)
                .append(mealReminderToBeSaved.mealHour)
                .append(mealReminderToBeSaved.mealMinute).toString()
            putExtra(MEAL_REMINDER_DATE_TIME, dateTimeConcat.toLong())
